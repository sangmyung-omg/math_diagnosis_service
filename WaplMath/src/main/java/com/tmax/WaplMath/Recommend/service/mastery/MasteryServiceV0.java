...
import com.tmax.WaplMath.Common.repository.knowledge.UserEmbeddingRepo;
...
public class MasteryServiceV0 implements MasteryServiceBase {
  ...
  @Autowired
  UserEmbeddingRepo userEmbeddingRepository;
  @Autowired
  @Qualifier("RE-ProblemUkRelRepo")
  ProblemUkRelRepo problemUkRelRepository;
  @Autowired
...
//			userEmbedding = userEmbeddingOptional.get().getUserEmbedding();
    ...
    for (int i = 0; i < probIdList.size(); i++) {
      ...
      String difficulty = problemRepository.findById(probIdInt).orElse(new Problem()).getDifficulty();
      if (difficulty == null) {

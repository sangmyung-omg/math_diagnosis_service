...
import com.tmax.Recommend.dao.ProblemDemoRepository;
import com.tmax.Recommend.dao.TypeUkRepository;
import com.tmax.Recommend.dao.UkRelRepository;
import com.tmax.Recommend.dao.UserDAO;
import com.tmax.Recommend.dao.UserExamCurriculumLogDAO;
import com.tmax.Recommend.dao.UserExamCurriculumLogRepository;
...
import com.tmax.Recommend.model.CardDTO;
import com.tmax.Recommend.model.ProblemDTO;
...
public class CurriculumCardListService {
	...
	public Integer addRecommendedCardInfo(String cardId, String cardType, String cardTitle, String userUuid,
		...
		UserExamCurriculumLogDAO newCard = new UserExamCurriculumLogDAO(cardId, cardType, cardTitle, userUuid,
	...
	public void addCardProblemInfo(CardDTO card) {
		...
		for (int i = 0; i < problemList.size(); i++) {
			...
			cardProblemMappingRepository.save(new CardProblemMappingDAO(cardProbId.getHigh(), cardId, "high",
					problem.getHigh(), null, new Integer(i + 1)));
			cardProblemMappingRepository.save(new CardProblemMappingDAO(cardProbId.getMiddle(), cardId, "middle",
					problem.getMiddle(), null, new Integer(i + 1)));
			cardProblemMappingRepository.save(new CardProblemMappingDAO(cardProbId.getLow(), cardId, "low",
					problem.getLow(), null, new Integer(i + 1)));
		}
	}

	// Load and generate CardDTO
	public CardDTO loadRecommendedCard(UserExamCurriculumLogDAO targetRecommendedCardLog) {
		...
		List<CardProblemMappingDAO> cardProblemList = cardProblemMappingRepository
		...
		for (CardProblemMappingDAO cardProblem : cardProblemList) {
	...
	public CardDTO generateTypeCard(String cardId, String typeUkId, String cardTitle, String userId) throws Exception {
		...
		if (ukUuidList.size() == 0)
			...
			for (String preUkUuid : preUkUuidList) {
				...
				Float preUkMastery = preUserKnowledgeDAO.getUkMastery();
	...
	public CardDTO generateSupplementCard(String cardId, String cardTitle, String userId,
			List<UserKnowledgeDAO> lowMasteryList) {
		...
		List<UkMasteryDTO> ukUuidMasteryList = new ArrayList<UkMasteryDTO>();
		for (UserKnowledgeDAO lowMasteryUK : lowMasteryList) {
	...
	public Map<String, Object> getCurriculumCardList(String userId, String date) throws Exception {
		...
		List<UserExamCurriculumLogDAO> targetRecommendedCardLogList = userExamCurriculumLogRepository
				.findAllByRecommendedDate(userId, targetDate);
		if (targetRecommendedCardLogList.size() != 0) {
			logger.info("\n이미 추천 기록이 있어서 불러옴");
			for (UserExamCurriculumLogDAO targetRecommendedCardLog : targetRecommendedCardLogList) {
		...
		if (targetDate.equals(examStartDate)) {
			...
			CurriculumDAO firstCurriculum = curriculumRepository.findNearestSection(currentCurriculumId);
		...
		List<UserKnowledgeDAO> lowMasteryList = userExamCurriculumLogRepository.findAllLowMasteryUkUuid(userId,
				lastSupplementCardSequence, supplementCardUkList, SUP_UK_MASTERY_THRESHOLD);

		logger.info("8. 보충 카드 제외, 유형 카드로 추천된 UK들의 이해도 리스트 = ");
		for (UserKnowledgeDAO lowMastery : lowMasteryList)
			logger.info(
